			...
			<div class="row-content">
				<?php 
					...
					if(isset($_GET['cat'])){
						...
						$select_sp_cat="select * from sp where sp_cat='$cat_id' order by sp_id limit $start,$numrow ";
									...
									<div class='thumbnail thumb-fix'>
										<img src='admin/image_admin/$sp_image' alt='error' >
										    <div class='caption'>
										     ...
										     <h3> Giá: $sp_gia </h3>
										     <p> $sp_chitiet </p>
										     <p><a href='cate.php?add_cart=$sp_id' class='btn btn-primary btn-fix' role='button'><button> Thêm vào giỏ hàng </button></a> <a href='detail.php?sp_id=$sp_id' class='btn btn-default' role='button'> Chi tiết </a></p>
										     </div>
										   </div>
										 </div>";
						}
					...
					<?php
					$sqldem="select count(*) as maxrow from sp where sp_cat='".$_SESSION['cat_id']."'";// đếm số dòng trong cơ sở dữ liệu
						...
						$linktrang="cate.php";// gán link hiển thị hoặc dùng $_SERVER["PHP_SELF"]
